...
export default class Henry extends Phaser.Scene {
    constructor() {
        super({key: 'Henry'});
    }
    preload(){
        this.load.image('button', './sprite/button.png');
        this.load.image("henry", "assets/henry.jpg");
    }
    create(){
        let background = this.add.sprite(0, 0, 'background_image').setOrigin(0).setDepth(0);
        background.setScale(2);
        this.henry = this.add.image(screenwidth * 0.5, screenheight * 0.25, 'henry');
        this.henry.setOrigin(0.5);
        this.bio_text = "By the end of this project, Henry will be 24. He realize he's getting old and cannot wait to graduate (again). " +
            "He's both tired and extremely enjoying school. \nHe just finished watching the last episode of Game of Thrones, which should be" +
            "retitled " +
            " as the Game of 8D Chess By the Three-Eyed Raven. Please don't consider that as spoilers as he's just salty. ";
